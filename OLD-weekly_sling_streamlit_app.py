...
# ---------------- UI ----------------
st.set_page_config(
    page_title="Weekly Sling Payroll – Streamlit", layout="wide")
...
with st.expander("Settings", expanded=False):
    THRESH_HOURS = st.number_input(
        "Zero if raw/trimmed exceeds (hours)", min_value=1.0, max_value=24.0, value=13.0, step=0.5)
    BREAK_DEDUCT_HOURS = st.number_input(
        "Unpaid break deducted (hours)", min_value=0.0, max_value=2.0, value=0.5, step=0.25)
    BREAK_MIN_SHIFT_HOURS = st.number_input(
        "Apply break only if Worked_final > (hours)", min_value=0.0, max_value=12.0, value=6.0, step=0.5)
    EMPLOYEE_WIDE_EXCLUSION = st.checkbox(
        "Exclude entire employee for week if any excluded row matches", value=False)
    excl_pattern_text = st.text_input(
        "Exclude rows where Position/Location matches (regex)", r"\bUHL\b|solid\s*fire|\badministration\b")
    try:
        ...
        excl_ok = False
    DISALLOW_WORKED_RAW_FALLBACK = st.checkbox(
        "Disallow fallback to 'SHIFT DURATION' when times are missing", value=False)
...
def generate_weekly_workbook_from_bytes(excel_bytes: bytes) -> tuple[bytes, dict]:
    ...
    df["has_clock_in"] = df["Clock In"].apply(
        lambda s: s.strip() not in {"", "-", "nan", "none", "None"})
    df["has_clock_out"] = df["Clock Out"].apply(
        lambda s: s.strip() not in {"", "-", "nan", "none", "None"})
    ...
    # Exclusions
    ex_mask = df[["Position", "Location"]].astype(str).apply(
        lambda c: c.str.contains(EXCLUDE_PATTERN, na=False))
    ...
    df["Break Applied (h)"] = np.where(df["Worked_final"] >
                                       BREAK_MIN_SHIFT_HOURS, BREAK_DEDUCT_HOURS, 0.0)
    ...
    # Weekly Totals
    holiday_by_emp = (
        audit.assign(_holiday=np.where(
            audit["VacTrainHoliday"], audit["Day"].map(full_day_for_dow).fillna(0.0), 0.0))
        .groupby("Employee")["_holiday"].sum()
    )
    weekly = (
        audit.groupby("Employee", as_index=False)["Policy Paid"].sum().rename(
            columns={"Policy Paid": "Computed Policy Total"})
    )
    ...
    qc_trimmed = audit[audit["eff_note"].astype(str).str.contains("Trimmed", na=False)][
        ["Employee", "Date", "Day", "Position", "Clock In", "Clock Out", "Sched Start",
            "Sched End", "Worked_final", "Worked_payable", "Policy Paid", "eff_note"]
    ]
    qc_zeroed = audit[audit["guard_zero"]][
        ["Employee", "Date", "Day", "Position", "Clock In", "Clock Out", "Worked_final",
            "Worked_payable", "Policy Paid", "over13_raw", "over13_eff", "no_clockout_noauto"]
    ]
    qc_breaks = audit[audit["Break Applied (h)"] > 0][
        ["Employee", "Date", "Day", "Position", "Worked_final",
            "Break Applied (h)", "Worked_payable", "Policy Paid"]
    ]
    ...
    # Exclusions view
    excluded_cols = [c for c in ["EMPLOYEE", "DATE", "POSITIONS", "LOCATIONS",
    ...
    ex_mask_raw = raw_in_week[["POSITIONS", "LOCATIONS"]].astype(
        str).apply(lambda c: c.str.contains(EXCLUDE_PATTERN, na=False))
    ...
    with pd.ExcelWriter(bio, engine="openpyxl") as writer:
        ...
        qc_vth_nopunch.to_excel(
            writer, sheet_name="QC - Leave no punches", index=False)
        excluded_view.to_excel(
            writer, sheet_name="QC - Excluded UHL+SF+Admin", index=False)
...
if go:
    if not uploaded:
        ...
        try:
            out_bytes, meta = generate_weekly_workbook_from_bytes(
                uploaded.read())
            st.success(f"Processed week: {meta['label']}")
            # Preview Weekly Totals
            st.subheader("Preview – Weekly Totals")
            st.dataframe(meta["weekly"].sort_values(
                "Employee"), use_container_width=True)
            st.download_button(
            ...
            ref = st.file_uploader(
                "Upload an expected/reference workbook (xlsx) to compare 'Weekly Totals'", type=["xlsx"], key="ref")
            if ref is not None:
                try:
                    exp_weekly = pd.read_excel(ref, sheet_name="Weekly Totals")
                    act_weekly = meta["weekly"].copy()
                    # Normalise names
                    exp_weekly["Employee"] = exp_weekly["Employee"].astype(
                        str).str.strip()
                    act_weekly["Employee"] = act_weekly["Employee"].astype(
                        str).str.strip()
                    # Pick common columns
                    common_cols = [c for c in ["Weekly Net Total", "Approved Pay Hours", "Computed Policy Total",
                                               "Holiday Hours"] if c in exp_weekly.columns and c in act_weekly.columns]
                    merged = exp_weekly[["Employee"] + common_cols].merge(
                        act_weekly[["Employee"] + common_cols], on="Employee", how="outer", suffixes=("_exp", "_act")
                    )
                    for c in common_cols:
                        merged[f"{c} diff"] = merged[f"{c}_act"] - \
                            merged[f"{c}_exp"]
                    # Filter mismatches
